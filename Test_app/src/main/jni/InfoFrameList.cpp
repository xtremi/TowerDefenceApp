...
void InfoFrameList::addListItem(const uString& name) {
	TextSprite* nameSprite = new TextSprite((char*)name.GetStr(), nameTextColor, glm::vec2(0), textSize);
	TextSprite* valueSprite = new TextSprite("", valueTextColor, glm::vec2(0), textSize);
	nameSprite->fixToScreen();
	valueSprite->fixToScreen();
	nameSprite->setDepth(3);
	valueSprite->setDepth(3);
	
	nameSprite->setFont(MEDIA_BANK.getFont(ASSET::FONT_MAIN));
	valueSprite->setFont(MEDIA_BANK.getFont(ASSET::FONT_MAIN));
